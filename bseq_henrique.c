...
int main(int argc, char *argv[])
{
  ...
  int numthreads = 4;
  char cromo[maxler][numthreads]; // Lê blocos para memória
  ...
  int tamanhos[25] = {0,
    ...
    for (f=1; f < 25; f++)
    {
		...
		#pragma omp parallel private(j) shared(m,cromo,tamanho,myseq,passoPercentual,percentagem) num_threads(numthreads)
		{
          for (j = 0; j < ((maxler/numthreads)-tamanho); j++)
          {
            ...
            {
              printf("Cromosoma %d: Similaridade de %.0f%% na posicao %d: ",f,similaridade,tid*(maxler/numthreads)j+m);
              for (k=0; k<tamanho; k++)
			  {
              printf("%c",cromo[j+k][tid]);
              printf("\n");
